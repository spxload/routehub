...
0. `/context` — записать, сколько контекста съедают коннекторы, плагины и
   скиллы; список лишних для RouteHub — Диане.
1. Проверить, что хук `guard-prod` срабатывает: попросить правку
   `routehub.conf` и убедиться, что пришёл запрос подтверждения (правку
   отклонить). Записать, в каком клиенте проверяли — веб или приложение
