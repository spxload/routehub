Сначала прочитай `studio/handoff/2026-09-24-pereezd-v-code.md`.
...
## Бриф первой сессии (добавлено 24.09)

0. Убедиться, что сессия запущена с ОДНИМ репозиторием `routehub`,
   подключённым при старте: иначе хуки `.claude/settings.json` не действуют
   (code.claude.com/docs/en/cloud-environments, «What carries over»;
   code.claude.com/docs/en/claude-projects, «What threads pick up»).
   `/context` — записать, сколько контекста съедают коннекторы, плагины и
   скиллы; список лишних для RouteHub — Диане.
1. Проверить, что хук `guard-prod` срабатывает: попросить правку
   `routehub.conf` через инструмент правки (не Bash — его хук не видит, это
   предохранитель, а не замок) и убедиться, что пришёл запрос подтверждения
   (правку отклонить). Записать, в каком клиенте проверяли — веб или приложение
   iOS: документация Claude Code не говорит, показывается ли запрос хука
   в мобильном приложении.
   Итог 24.09 (веб claude.ai в Safari на iPhone, режим auto): запрос хука
   пришёл, Диана отклонила — хук работает. Первая попытка в той же сессии
   прошла без отклонения (правка сразу откачена, в ветку не попала);
   причина — не могу подтвердить.
2. Залить два файла ниже: ветка сессии → PR → Merge Дианы (`git push` в
   облачной сессии работает только в ветку сессии, см. `CLAUDE.md`).
