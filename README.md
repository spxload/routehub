...
**Состояние на 2026-09-21:** Worker `v1.11.0`, конфиг `C-draft-43`,
спидтест `v0.7.0`, панель устройства `dash v0.8.0`, админ-панель `p3`. Проверить живой Worker: `curl https://<worker>/version`.
...
| `docs/ADR-03_ЖУРНАЛ_ПО_ПРИЛОЖЕНИЮ.md` | что делать с `processPath`/`pid` в Stash: журнал и маршрутизация по приложению |
| `docs/ADR-05_ЗАМЕР_ОТДАЧИ.md` | замер отдачи (`up`): куда, сколько трафика, почему пока не в балле |
...
`docs/ADR-02_ГРУППЫ_STASH.md`, `docs/ADR-03_ЖУРНАЛ_ПО_ПРИЛОЖЕНИЮ.md`, `docs/ADR-05_ЗАМЕР_ОТДАЧИ.md`, `docs/ТЕХДОЛГ.md`,
...
node --test "tests/*.test.js"          # 120 тестов, CI нет намеренно
