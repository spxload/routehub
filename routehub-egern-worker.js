...
// VERSION: e0.2.3 (2026-08-08) — ШАГ 4.3: диагностическая группа RH-Проба-DIRECT.
//   Базис — e0.2.2 (шаг 4.2: узлы + минимальный профиль), принят на k3
//   2026-08-08: 5 групп, 52 узла в обычном пуле, 17 в обходе, предупреждений нет.
//   Решения и границы объёма — ЭТАП_K_ШАГ_4.2.md (эта же ветка), п. 8.
...
//   На 2026-08-08 обычный вариант принят устройством, то есть форма поля верна;
//   safe сохранён как инструмент для последующих правок.
// ПРО RH-Проба-DIRECT (e0.2.3): единственная группа с членом DIRECT.
//   Ни одно правило на неё не ссылается — маршрутизация не меняется.
//   Назначение: проверки K1 и K2. Профиль задаёт два разных адреса пробы —
//   direct_latency_test_url (msftconnecttest, из РФ достижим всегда) и
//   групповой latency_test_url (cp.cloudflare.com). Наблюдаемое: получает ли
//   член DIRECT результат пробы и по какому из адресов. Если по первому —
//   под whitelist DIRECT всегда выглядит живым и модель «слабый DIRECT»
//   (вывод 2 в СТАРТ.md) на Egern не переносится.
...
// журналом решений этапа K. Порядок — как отдал провайдер (сортировки нет).
...
function buildProfile(base, key, safe) {
  ...
  const groups = [
    ...
    // Диагностика K1/K2 (шаг 4.3). Единственная группа с членом DIRECT;
    // ни одно правило на неё не ссылается, маршрутизация не меняется.
