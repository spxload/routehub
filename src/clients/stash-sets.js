...
const SETS = [
  ...
  // ✅ ЗАМЕР 02.09: ДОЕХАЛ ЦЕЛИКОМ — 81 061 правило. Хвост 3 закрыт.
  // Предел Stash — ПО БАЙТАМ, не по строкам: 81 061 строка при 1 327 481
  // байте прошла, 81 758 строк при 2 483 144 байтах — нет. Порог лежит
  // между 1,33 и 2,48 МБ; точнее не мерили и незачем.
  // Это ПОЛНАЯ замена отключённому `rh-refilter`: тот же список
  // заблокированного в РФ, тот же первоисточник, только голыми доменами и
  // потому вдвое компактнее.
  // Прежний комментарий (эксперимент) оставлен ниже как история решения.
  // ⚗️ БЫЛО ЭКСПЕРИМЕНТОМ, а не готовым решением: Тот же список
