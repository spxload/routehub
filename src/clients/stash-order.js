...
const CTRL_MAX = 0x20, CTRL_DEL = 0x7f;

// Имя узла для YAML. Управляющий символ заменяется пробелом: в одинарной
// строке YAML он недопустим и порвал бы разбор ВСЕГО профиля, а не одной
// группы. Затем norm схлопывает пробелы и обрезает края.
// ЭТО ЖЕ ДЕЛАЕТ nodeName в stash-nodes.js — иначе имя члена группы не совпало
// бы с именем узла в `proxies:` и Stash молча выкинул бы узел из группы.
// Расхождение осталось одно: stash-nodes.js не схлопывает подряд идущие
// ОБЫЧНЫЕ пробелы, а norm схлопывает. В именах подписки Lastdep двойных
// пробелов нет (срез 2026-08-24); проверить на стенде при смене состава.
function cleanName(s) {
  let out = '';
  for (const ch of String(s)) {
    const c = ch.codePointAt(0);
    out += (c < CTRL_MAX || c === CTRL_DEL) ? ' ' : ch;
  }
  return norm(out);
}
...
function collect(masterLines, state, label) {
  ...
  (masterLines || []).forEach(function (line) {
    ...
    items.push({ base: base, tag: tag, flag: flagOf(base), w: w, c: c, display: base });
  });
