...
import { BLK, BL_CAP, BYPASS_WORD, CELL_HINTS, DEAD, FLAG_RE, FLAG_START_RE, FLOOR_BL, FLOOR_JIT, FLOOR_RTT, JIT_CAP, METRIC_SEP, PROX, REGION_AM, REGION_EU, REGION_RU, RH_ICON_SVG, SCORE_WB, SCORE_WJ, SCORE_WR, SCORE_WS, SUP_DIG, SUP_PLUS, VOICE, VOICE_BL, VOICE_JIT, VOICE_MED } from './const.js';
...
// Поля `up` (отдача, v1.11.0) здесь нет НАМЕРЕННО — ADR-05: веса проверены
// 16.08 и оставлены до появления метрики с настоящим разбросом; вводить `up`
// в балл можно только по накопленным данным и отдельным решением. Когда это
// случится: отсутствие `up` обязано давать ХУДШИЙ или нейтрально-средний
// компонент, но не лучший — дефект 19.09 с `jit: null` (jN = 1) повторять
// нельзя. Тест «up не влияет на балл» держит это место.
...
// v1.11.1: обход — слово BYPASS_WORD в ЛЮБОМ месте имени, а не подстрока
// '[Обход'. Согласовано с RH-Filter-Обход (NameKeyword) и «^(?!.*Обход)»
// VPN-фильтров конфига, а также с isBypass() скрипта спидтеста (v0.7.0).
// Ошибка в эту сторону (обычный узел со словом сочтён обходом) стоит узла,
// ошибка в обратную — платного трафика (правило 1), поэтому признак широкий.
...
function metricOf(s) {
  ...
  // v1.11.0: ОТДАЧА (`up`, Мбит/с, один знак) — ADR-05. Только хранится и
  // показывается, в scoreOf и voiceOk НЕ входит: сначала меряем, потом
  // решаем по данным. Поле пишется только когда пришло ЧИСЛОМ: отсутствие
  // замера остаётся отсутствием поля и НИКОГДА не становится ни 0 («отдача
  // не прошла»), ни хорошим значением. 0 — это настоящий результат:
  // устройство шлёт его, когда тело не ушло целиком.
  if (typeof s.up === 'number' && isFinite(s.up)) o.up = Math.max(0, Math.round(s.up * 10) / 10);
  return o;
}
