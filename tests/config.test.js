...
// Техдолг 6.
//
// С v1.11.0 ветки (T-private-repo, перенос из main v1.12.0) шаблон —
// НАСТОЯЩИЙ routehub.conf, встроенный в сборку (src/files.js; в тестах его
// подставляет tests/text-loader.mjs).
...
// ── Профиль Stash (CLIENT=stash, usesTemplate = false) ─────────────────
// Стенд routehub-stash отдаёт профиль, собранный кодом. Шаблон ему не нужен,
// а после перевода репозитория в приватный любая прямая ссылка на
// spxload/routehub в профиле — 404 на устройстве. Переписчик к профилю НЕ
// применяется (логика профиля не менялась), поэтому тест проверяет сам
// профиль: ссылок на spxload в нём нет вовсе, в сеть /config не ходит.
function envStash(extra) {
  return makeEnv(Object.assign({
    sub_cache: SUB,
    devices: { k1: { status: 'bound', token: TOKEN, cell_unlim: true, ewma: false } },
  }, extra || {}), { CLIENT: 'stash' });
...
test('/config стенда Stash: профиль без ссылок на spxload и без запросов к сети', async () => {
  const env = envStash();
  await withStubbedFetch(async (seen) => {
    const r = await worker.fetch(get('k1', TOKEN), env);
    assert.equal(r.status, 200, await r.clone().text());
    const text = await r.text();
    assert.ok(text.indexOf('proxy-groups:') >= 0, 'предпосылка: это профиль Stash');
    assert.equal(text.indexOf('spxload'), -1, 'ссылка на spxload в профиле Stash');
    assert.deepEqual(seen, [], 'fetch из /config Stash: ' + seen.join(', '));
    // В профиле токен устройства (/t/<токен>/nodes) — кэшам не хранить.
    assert.equal(r.headers.get('Cache-Control'), 'no-store');
  });
});

// Профиль собирается и без токена в пути (фаза token_required=false) — та же
// проверка, чтобы ветка без токена не оказалась иной.
test('/config стенда Stash без токена в пути: ссылок на spxload нет', async () => {
  const env = envStash();
  await withStubbedFetch(async () => {
    const r = await worker.fetch(get('k1'), env);
    assert.equal(r.status, 200);
    assert.equal((await r.text()).indexOf('spxload'), -1);
  });
});
