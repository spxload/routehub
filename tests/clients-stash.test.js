...
// ── ЗАМЕР ЗАДЕРЖКИ ЯДРОМ (S-draft-3, пересмотрен в S-draft-5) ───────
// S-draft-3 выключал замер обходных узлов ключом `benchmark-disabled` ради
// правила 1 («обходные узлы не тестировать — платный трафик»). Замер на
// устройстве 03.09 показал, что цена этого была не та, что считали: обходные
// узлы перестали работать ВООБЩЕ — ни задержки при ручном тесте, ни
...
test('замер задан всем узлам, включая обходные, и benchmark-disabled нигде нет', () => {
  const yaml = S.renderNodes(LINES, STATE, {});
  const blocks = yaml.split(/\n(?=\s*- name:)/);
  let seenBypass = 0, seenNormal = 0;
  blocks.forEach(function (b) {
    if (b.indexOf('name:') < 0) return;
    if (b.indexOf('Обход') >= 0) seenBypass++; else seenNormal++;
    assert.ok(b.indexOf('benchmark-url:') >= 0, 'у узла нет адреса теста');
    assert.ok(b.indexOf('benchmark-timeout:') >= 0, 'у узла нет тайм-аута теста');
    assert.ok(b.indexOf('benchmark-disabled') < 0,
      'вернулся benchmark-disabled — на устройстве такой узел не работает вовсе');
  });
...
// S-draft-6: у обходных узлов СВОЙ тайм-аут замера. Не из осторожности, а по
// числам ST14 (04.09): рабочий узел ответил за 70 мс, живой обходной — за
// 729 мс, то есть в десять раз дольше; при общем тайм-ауте в 3 с замер ходил
// около порога, и обходные узлы «то отвечают, то нет». Тест сторожит обе
// стороны: возврат общего тайм-аута для обхода и его расползание на обычные
// узлы (там длинный тайм-аут задерживал бы всю проверку группы).
test('у обходных узлов свой, более длинный тайм-аут замера', () => {
  const yaml = S.renderNodes(LINES, STATE, {});
  const blocks = yaml.split(/\n(?=\s*- name:)/);
  let b = 0, n = 0;
  blocks.forEach(function (blk) {
    if (blk.indexOf('name:') < 0) return;
    const m = blk.match(/benchmark-timeout:\s*(\d+)/);
    assert.ok(m, 'у узла нет тайм-аута замера');
    if (blk.indexOf('Обход') >= 0) {
      b++;
      assert.equal(m[1], String(S.BENCH_TIMEOUT_BYPASS),
        'обходному узлу вернули общий тайм-аут — замер снова будет ходить около порога');
    } else {
      n++;
      assert.equal(m[1], String(S.BENCH_TIMEOUT),
        'обычному узлу задан длинный тайм-аут — он задержит проверку всей группы');
    }
  });
  assert.ok(b > 0 && n > 0, 'в наборе нет обеих разновидностей узлов');
  assert.ok(S.BENCH_TIMEOUT_BYPASS > S.BENCH_TIMEOUT, 'тайм-аут обхода не длиннее обычного');
});
...
// S-draft-5: `lazy` у группы обхода СНЯТ вместе с `benchmark-disabled`.
// Он вводился вторым рубежом под то же правило 1, а обход в итоге не работал
// вовсе. Пока причина не подтверждена окончательно, оба ключа сняты разом:
// разбирать, какой из двух виноват, имеет смысл только после того, как обход
// заработает хоть в каком-то виде. Правило 1 держит интервал замера.
test('у групп задан интервал замера, а lazy нигде нет', () => {
  ...
  assert.equal(bypass.lazy, undefined, 'вернулся lazy — снят вместе с benchmark-disabled');
