...
test('эталон читается: в боевом конфиге 32 локальных правила и 12 удалённых наборов', () => {
  // 24 -> 30 (08.09): четыре доменных правила Apple для App Store и iMessage
  // плюс две узкие строки над ними.
  // 30 -> 32 (19.09, C-draft-43): yt3/yt4.googleusercontent.com — аватары и
  // шапки каналов YouTube, которых нет ни в одном из удалённых наборов.
  // Число сторожит парность контуров — правка одного Loon без Stash
  // (или наоборот) роняет этот тест, что уже и произошло при внесении.
  assert.equal(CONF_LOCAL.length, 32, '[Rule] боевого конфига изменилась — сверить перенос');
...
// 02.09: ДОПОЛНЕНИЙ К [Rule] БОЛЬШЕ НЕТ. Пять доменов инфраструктуры
// YouTube, стоявших здесь с 31.08, сняты: набор `rh-youtube` покрывает их
// точными правилами (замер ST8). Зеркало восстановлено — локальные правила
// профиля снова построчно повторяют боевой конфиг.
// Механизм оставлен НАРОЧНО: следующее дополнение объявляется добавлением
// строки сюда, а не правкой сверки. Пустой список тоже утверждение — он
// говорит «расхождений нет», и тест это проверяет.
const LOCAL_EXTRA = [];
