// Сторож манифеста встроенных файлов src/files.js (T-private-repo, v1.12.0).
//
...
test('манифест: каждый файл scripts/*.js, probes/*.js, plugins/*.plugin|*.stoverride и routehub.conf встроен', () => {
  assert.ok(ON_DISK.length >= 20, 'подозрительно мало файлов на диске: ' + ON_DISK.length);
...
test('wrangler.toml: правила Text боевого и стенда одинаковы и покрывают манифест', () => {
  ...
  for (const p of ['routehub-worker.js', 'src/api.js', 'src/repo.js', 'src/files.js', 'src/clients/loon.js']) {
...
test('ссылки на spxload/routehub в конфиге и плагинах ведут только на встроенные файлы', () => {
  const re = /https:\/\/(?:raw\.githubusercontent\.com\/spxload\/routehub\/main|cdn\.jsdelivr\.net\/gh\/spxload\/routehub@main)\/([\w./-]+)/g;
  const bad = [];
  for (const p of Object.keys(FILES)) {
